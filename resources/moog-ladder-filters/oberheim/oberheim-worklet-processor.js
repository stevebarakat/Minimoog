...
/**
 * Oberheim Variation Moog filter worklet processor
 * Based on Will Pirkle's virtual analog approach
 * Uses 4 cascaded one-pole filters with feedback
 */
class OberheimWorkletProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    ...
    this.port.onmessage = (e) => {
      ...
      if (e.data instanceof ArrayBuffer) {
        WebAssembly.instantiate(e.data)
          ...
          .catch((error) => {
            console.error(
              "Oberheim processor: WASM instantiation failed:",
              error
            );
...
registerProcessor("oberheim-worklet-processor", OberheimWorkletProcessor);
