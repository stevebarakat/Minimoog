...
export function customRender(
  ...
  const Wrapper = ({ children }: { children: React.ReactNode }) => {
    ...
    if (withToast) {
      content = <ToastProvider>{children}</ToastProvider>;
    }

    return <RadixTooltip.Provider>{content}</RadixTooltip.Provider>;
  };
...
export function renderWithProviders(
  ui: React.ReactElement,
  options?: Omit<RenderOptions, "wrapper"> & { withToast?: boolean }
) {
  const { withToast = false, ...renderOptions } = options || {};
...
export function createMockSynthState(
  ...
  return {
    ...
    filterCutoff: createFilterCutoffRange(5),
    ...
    effectsVolume: createVolumeRange(7),

    // Delay effect state
    delay: {
      enabled: false,
      mix: createVolumeRange(3),
      time: createFilterEnvelopeRange(2),
      feedback: createVolumeRange(2),
    },

    // Reverb effect state
    reverb: {
      enabled: false,
      mix: createVolumeRange(5),
      decay: createFilterEnvelopeRange(5),
      tone: createFilterEnvelopeRange(5),
    },
