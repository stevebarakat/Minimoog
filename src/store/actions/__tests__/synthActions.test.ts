...
describe("synthActions", () => {
  describe("loadPreset", () => {
    ...
    it("should load effects correctly from preset", () => {
      const mockSet = vi.fn();
      const actions = createSynthActions(mockSet);

      const initialState = createInitialState();
      const preset = {
        effectsVolume: createVolumeRange(5),
        delay: {
          enabled: true,
          mix: createVolumeRange(6),
          time: createFilterEnvelopeRange(3),
          feedback: createVolumeRange(4),
        },
        reverb: {
          enabled: true,
          mix: createVolumeRange(8),
          tone: createFilterEnvelopeRange(6),
        },
      };
      ...
      // Verify that effects are loaded correctly
      expect(capturedState!.effectsVolume).toBe(5);
      expect(capturedState!.delay).toEqual({
        enabled: true,
        mix: 6,
        time: 3,
        feedback: 4,
      });
      expect(capturedState!.reverb).toEqual({
        enabled: true,
        mix: 8,
        tone: 6,
      });
