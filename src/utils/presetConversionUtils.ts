 ...
 * Transforms the external preset structure into the internal synth store format,
 * applying appropriate range validation and type conversion for each parameter.
 *
 * @param preset - The preset object containing all synthesizer parameters
 * @returns Partial SynthState object with converted and validated parameters
 *
 * @example
 * ```typescript
 * const preset = { controllers: { tune: 5 }, filter: { filterCutoff: 1000 } };
 * const storeState = convertPresetToStoreFormat(preset);
 * // Returns: { masterTune: 5, filterCutoff: 1000, ... }
 * ```
 */
export function convertPresetToStoreFormat(
  ...
  return {
    ...
    // Effects volume
    ...(preset.effects?.volume !== undefined && {
      effectsVolume: createVolumeRange(preset.effects.volume),
    }),

    // Effects
    ...(preset.effects?.delay && {
      delay: {
        enabled: preset.effects.delay.enabled,
        mix: createVolumeRange(preset.effects.delay.mix),
        time: createFilterEnvelopeRange(preset.effects.delay.time),
        feedback: createVolumeRange(preset.effects.delay.feedback),
      },
    }),
    ...(preset.effects?.reverb && {
      reverb: {
        enabled: preset.effects.reverb.enabled,
        mix: createVolumeRange(preset.effects.reverb.mix),
        tone: createFilterEnvelopeRange(preset.effects.reverb.tone),
      },
    }),
 ...
 * Transforms the internal synth store state into the external preset format,
 * handling optional effects and providing default values for missing parameters.
 *
 * @param state - The current synth store state
 * @param id - Unique identifier for the preset
 * @param name - Human-readable name for the preset
 * @param description - Detailed description of the preset sound
 * @param category - Category classification for organizing presets
 * @returns Preset object with all synthesizer parameters in external format
 *
 * @example
 * ```typescript
 ...
 *   "bass-001",
 *   "Deep Bass",
 *   "Warm analog bass with filter sweep",
 *   "Bass"
 * );
 * ```
 */
export function convertStateToPresetFormat(
  ...
  return {
    ...
    effects: {
      ...(state.effectsVolume !== undefined && {
        volume: state.effectsVolume || 7,
      }),
      ...(state.delay && {
        delay: {
          enabled: state.delay.enabled,
          mix: state.delay.mix || 0,
          time: state.delay.time || 0,
          feedback: state.delay.feedback || 0,
        },
      }),
      ...(state.reverb && {
        reverb: {
          enabled: state.reverb.enabled,
          mix: state.reverb.mix || 0,
          tone: state.reverb.tone || 0,
        },
      }),
    },
  };
