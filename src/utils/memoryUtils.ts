...
export function monitorMemoryUsage(thresholdMB = 100): void {
  ...
  if (stats) {
    ...
    if (usedMB > thresholdMB) {
      ...
      if (usedMB > thresholdMB * 1.5) {
        logger.warn("Triggering aggressive memory cleanup due to high usage");
...
/**
 * Trigger aggressive memory cleanup when usage is very high
 */
function triggerAggressiveCleanup(): void {
  logger.warn("=== AGGRESSIVE MEMORY CLEANUP TRIGGERED ===");
  ...
  // Only clear caches, don't dispose optimizers that are needed for functionality
  try {
    // Clear audio buffer cache only
    import("@/utils/audioBufferOptimization").then(
      ({ getAudioBufferOptimizer }) => {
        const optimizer = getAudioBufferOptimizer();
        if (optimizer) {
          logger.warn("Clearing audio buffer cache...");
          optimizer.clearCache();
        }
      }
    );

    // Clean up node pool but don't dispose it completely
    import("@/utils/nodePoolingUtils").then(({ cleanupPool }) => {
      logger.warn("Cleaning up node pool...");
      cleanupPool();
      // Don't dispose the entire pool - just clean up inactive nodes
    });

    // Don't dispose any optimizers - they're needed for functionality
    // Just clear their internal caches if they have any
    logger.warn("Skipping optimizer disposal to prevent recreation loops");
  } catch (error) {
    logger.warn("Error during aggressive cleanup:", error);
  }

  logger.warn("=== AGGRESSIVE CLEANUP COMPLETE ===");
}
