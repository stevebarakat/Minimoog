...
vi.mock("@/store/selectors", async (importOriginal) => {
  ...
  return {
    ...actual,
    useKeyboardState: vi.fn(() => ({ activeKeys: null })),
    useOscillator3ControlsState: vi.fn(() => ({
      osc3FilterEgSwitch: false,
      noiseLfoSwitch: false,
    })),
    useIsSynthDisabled: vi.fn(() => false),
    useMasterControlsState: vi.fn(() => ({
      masterTune: 0,
    })),
  };
});
...
vi.mock("../hooks/useKeyboardControl", () => ({
  useKeyboardControl: vi.fn(),
}));

vi.mock("../hooks/useUIState", () => ({
  useUIState: vi.fn(() => ({ isMobile: false, view: "desktop" })),
}));

vi.mock("../hooks/useAudioContextManagement", () => ({
  useAudioContextManagement: vi.fn(() => ({
    audioContext: null,
    isInitialized: false,
    onInitialize: vi.fn(),
    onDispose: vi.fn(),
...
vi.mock("../hooks/useScrollControls", () => ({
  useScrollControls: vi.fn(() => ({
...
describe("Minimoog - Integration Tests", () => {
  ...
  it("updates display when state changes", () => {
    ...
    expect(allKnobs.length).toBeGreaterThan(0);

    // Check that knobs are interactive and have proper accessibility attributes
    allKnobs.forEach((knob) => {
      // Check that the knob has proper role and is focusable
      expect(knob).toHaveAttribute("role", "slider");
      expect(knob).not.toBeDisabled();

      // Check for any value-related attributes (optional)
      const hasAriaValueNow = knob.hasAttribute("aria-valuenow");

      // If any value attribute exists, it should be valid
      if (hasAriaValueNow) {
