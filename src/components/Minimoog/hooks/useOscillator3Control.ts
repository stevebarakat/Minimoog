...
export function useOscillator3Control(
  ...
  const calculateFreeRunningFrequency = useCallback(() => {
    ...
    // For modulation purposes, we want musical frequencies suitable for LFO-like effects
    const baseFreq = 10; // Musical frequency for vibrato effect (matches modulation sources)
    const rangeMultiplier = rangeToMultiplier[oscillator3.range] || 1;

    // Expand frequency range: -7 to +8 semitones becomes wider range
    // Map -7 to +8 to give a good range for modulation (0.5Hz to 50Hz)
    const frequencyMultiplier = Math.pow(2, oscillator3.frequency / 1.5); // Wider range than keyboard control

    return Math.max(
      0.5,
      Math.min(50, baseFreq * rangeMultiplier * frequencyMultiplier)
    );
  ...
  useEffect(() => {
    ...
    // Cleanup function
    return () => {
      // Cleanup is now handled at the beginning of the effect
    };
  }, [
    audioContext,
    osc3Control,
