...
export function useModulation({
  ...
  useEffect(() => {
    ...
    try {
      const lfo = getPooledNode("oscillator", audioContext);
      ...
      const lfoGain = getPooledNode("gain", audioContext);
  ...
  useOscillatorModulation({
    ...
    lfoNode: lfoNodeRef.current, // Pass the LFO node for vibrato effect
  });
  ...
  // Return the modulation signal function for use by other hooks
