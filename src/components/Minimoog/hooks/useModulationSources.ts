...
export function useModulationSources() {
  const noiseGeneratorsRef = useRef<NoiseGenerators>({
    white: createWhiteNoiseGenerator(),
    ...
    (time: number): number => {
      ...
      const source1 = osc3FilterEgSwitch
        ...
        : (() => {
            ...
            // Authentic Minimoog uses 8-12Hz base frequency for musical vibrato
            const baseFreq = 10; // Musical frequency for vibrato effect
      ...
      const source2 = noiseLfoSwitch
        ...
        ? noiseGeneratorsRef.current.white()
