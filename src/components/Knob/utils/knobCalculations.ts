...
export function getKnobSensitivity(
  max: number,
  min: number,
  size: "small" | "medium" | "large"
): number {
  const parameterRange = max - min;
  const knobDiameter = size === "small" ? 32 : size === "medium" ? 40 : 48;
  return (parameterRange / 100) * (196 / knobDiameter);
}
