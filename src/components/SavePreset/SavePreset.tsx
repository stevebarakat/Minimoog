...
import { convertStateToPresetFormat } from "@/utils/data";
...
function SavePreset({ disabled = false }: SavePresetProps) {
  ...
  const handleSavePreset = useCallback(async () => {
    ...
    effects: {
      volume: ${preset.effects?.volume || 7},
      delay: {
        enabled: ${preset.effects?.delay?.enabled || false},
        mix: ${preset.effects?.delay?.mix || 0},
        time: ${preset.effects?.delay?.time || 0},
        feedback: ${preset.effects?.delay?.feedback || 0},
      },
      reverb: {
        enabled: ${preset.effects?.reverb?.enabled || false},
        mix: ${preset.effects?.reverb?.mix || 0},
        tone: ${preset.effects?.reverb?.tone || 0},
      },
    },
  },`;
