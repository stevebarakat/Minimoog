...
export function useCopyUrl() {
  ...
  const effectsVolume = useSynthStore((state) => state.effectsVolume);
  const delay = useSynthStore((state) => state.delay);
  const reverb = useSynthStore((state) => state.reverb);
  ...
  const handleCopyURL = async () => {
    try {
      ...
      const msg = `Failed to copy preset URL: ${errorMessage}`;
