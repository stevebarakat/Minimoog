...
export const rangeToMultiplier: Record<BaseOscillatorParams["range"], number> =
  {
    "32": 0.25,
    "16": 0.5,
    "8": 0.5, // Authentic Minimoog tuning - one octave lower than standard
    "4": 2,
    "2": 4,
...
export function createBaseOscillator(
  ...
  if (mixerNode) {
    // Connect to specific mixer input channel based on oscillator type
    // This will be set by the calling code when creating the oscillator
  ...
  function setFrequency(frequency: number): void {
    if (osc) {
      ...
      // Use smooth frequency transitions to prevent popping
      const now = params.audioContext.currentTime;
      const transitionTime = 0.005; // 5ms smooth transition
