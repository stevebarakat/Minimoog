    ...
    (note: string) => {
      ...
      if (!glideOn && oscRef.current) {
        try {
          ...
          console.error("Error cleaning up existing oscillator:", error);
        }
      }
      ...
      try {
        ...
        if (!oscNode) {
          ...
          // Even when glide is off, use a very short transition to prevent popping
          const rangeMultiplier = oscRef.current.getRangeMultiplier();
          const transitionTime = 0.003; // 3ms transition

          oscNode.frequency.cancelScheduledValues(audioContext.currentTime);
          oscNode.frequency.setValueAtTime(
            oscNode.frequency.value,
            audioContext.currentTime
          );
          oscNode.frequency.linearRampToValueAtTime(
            frequency * rangeMultiplier,
