...
import { createMockStore, renderWithProviders } from "@/test/testHelpers";
...
describe("Modifiers - Integration Tests", () => {
  ...
  it("renders all modifier sections with their controls", () => {
    renderWithProviders(<Modifiers />, { withToast: true });
  ...
  it("supports keyboard navigation through all controls", async () => {
    ...
    await user.click(filterModSwitch);
    // Wait for requestAnimationFrame to complete before checking focus
    await new Promise((resolve) => requestAnimationFrame(resolve));
    // The label element should have focus (as designed by the RockerSwitch component)
    const filterModulationInput = document.querySelector("#filter-modulation");
    expect(filterModulationInput).toBeInTheDocument();
    const filterModulationLabel = filterModulationInput!.closest("label");
    expect(filterModulationLabel).toHaveFocus();
  });
